#!/bin/bash
# Wait for two docker healthchecks to be in a "healthy" state before executing a "docker exec $2 bash $3"
##############################################################################################################################
...
while [ $attempt -le 79 ]; do
  ...
  if [[ $health1 == "healthy" && $health2 == "healthy"  ]]; then
    ...
    if [[ $3 == 'npm_deploy' ]]; then
      ...
      docker exec $2 bash -c '[[ -n "$GITHUB_TOKEN" ]] && { echo "GITHUB_TOKEN found"; } || echo "!!!!!!!!!!!!! GITHUB_TOKEN not found !!!!!!!!!!!!!"'

      CMD="npm run jsdocp-deploy"
      #docker exec $2 bash -c '[[ -z "$GITHUB_TOKEN" ]] && { echo "Missing GITHUB_TOKEN" >&2; exit 1; }'
      #[[ $? != 0 ]] && { echo "Failed to \"$3\" at \"$CMD\" in docker container \"$2\": Missing GITHUB_TOKEN env var in container" >&2; exit 1; }
      docker exec $2 bash -c "git pull"
      [[ $? != 0 ]] && { echo "Failed to \"$3\" at \"$CMD\" for \"git pull\" in docker container \"$2\"" >&2; exit 1; }
      docker exec $2 bash -c "$CMD"
      [[ $? != 0 ]] && { echo "Failed to \"$3\" at \"$CMD\" in docker container \"$2\"" >&2; exit 1; }

      docker exec $2 bash -c '[[ -n "$NPM_TOKEN" ]] && { echo "NPM_TOKEN found"; } || echo "!!!!!!!!!!!!! NPM_TOKEN not found !!!!!!!!!!!"'

      CMD="npm publish"
      #docker exec $2 bash -c '[[ -z "$NPM_TOKEN" ]] && { echo "Missing NPM_TOKEN" >&2; exit 1; }'
      #[[ $? != 0 ]] && { echo "Failed to \"$3\" at \"$CMD\" in docker container \"$2\": Missing NPM_TOKEN env var in container" >&2; exit 1; }
      docker exec $2 bash -c 'echo "//registry.npmjs.org/:_authToken=\${NPM_TOKEN}" > .npmrc'
      [[ $? != 0 ]] && { echo "Failed to \"$3\" at \"$CMD\" in docker container \"$2\": Failed to write .npmrc" >&2; exit 1; }
      docker exec $2 bash -c "$CMD"
      [[ $? != 0 ]] && { echo "Failed to \"$3\" at \"$CMD\" in docker container \"$2\"" >&2; exit 1; }
    else
      docker exec $2 bash -c "$3"
